  ...
  return (
    <>
      <div className='Mec2'>
        ...
        {!!menuData[0] && menuData[0].length ? menuData[0].filter((v,i)=>
            v.FoodType === 'Main').map((value,index)=>{

              console.log("menu3Main:",index);
            return( <React.Fragment key={value.RandomKey}>
          <M3Main value={value} index={index} arrayData={arrayData} />
          </React.Fragment>
          )}):null}
        </div>
          ...
          {!!menuData[0] && menuData[0].length ? menuData[0].filter((v,i)=>
            v.FoodType === 'Main').map((value,index)=>{
              ...
              return (
                  ...
                  <M3Dot value={value} index={index} ClicktoChange={()=>{setArrayData(index)}} arrayData={arrayData} key={index}/>
